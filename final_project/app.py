...
def history():
    ...
    user_id = session["user_id"]
    return render_template("history.html", transactions = db.execute("SELECT * FROM transactions WHERE user_id = :id", id = user_id))
...
def add_cash():
    ...
    if request.method == "GET":
        ...
        if not new_cash:
            return apology("Type in a number to add")
